    ...
    if(isset($_GET["nam"]) AND isset($_GET["thang"])){
        ...
        if($thang=='all' AND $nam=='all'){
           ...
           cb.MADV='$madv'";
            ...
            $sqlqureryluong="SELECT lt.*, cb.HOTENCB,cb.CHUCVU, dv.TENDV FROM donvi dv, luongchitiet lt,
            canbo cb WHERE lt.MACB=cb.MACB AND dv.MADV=cb.MADV AND `Nam` = $nam AND cb.MADV='$madv'";
        } else if($nam=='all'){
            $sqlqureryluong="SELECT lt.*, cb.HOTENCB,cb.CHUCVU, dv.TENDV FROM donvi dv, luongchitiet lt,
            canbo cb WHERE lt.MACB=cb.MACB AND dv.MADV=cb.MADV AND
            `Thang` = $thang  AND cb.MADV='$madv'";
        }else {
            $sqlqureryluong="SELECT lt.*, cb.HOTENCB,cb.CHUCVU, dv.TENDV FROM donvi dv, luongchitiet lt,
            canbo cb WHERE lt.MACB=cb.MACB AND dv.MADV=cb.MADV AND
            `Thang` = $thang AND `Nam` = $nam AND cb.MADV='$madv'";
        }
...
<body style="background: #FFFF; padding: 0px; margin: 0px;">
   <div class="Container">
   <div class="row">
    <div id="header">
...
<div style="text-align: center"><h2>Bảng lương</h2></div>
    <div class="row">
        ...
        <div class="col-md-5">
            <form action="QLLuongNv.php" method="GET" class="form-inline">
...
<table class="table table-striped table-bordered table-hover" id="dataTables">
    ...
    echo '<a target="_blank" href="QLluongNvDayDu.php?thang='.$thang . '&nam='.$nam.'">Xem bảng lương chi tiết</a>';
    ?>
    </div>
</div>
   <link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/bs/dt-1.10.16/af-2.2.2/b-1.4.2/cr-1.4.1/fc-3.2.3/fh-3.1.3/kt-2.3.2/r-2.2.0/rg-1.0.2/rr-1.2.3/sc-1.4.3/sl-1.2.3/datatables.min.css"/>
    ...
    $(document).ready(function() {
        ...
        order: [[ 0, 'asc' ]],
				'language': {
                    'info': 'Hiển thị _START_ đến _END_ của _TOTAL_ Người',
